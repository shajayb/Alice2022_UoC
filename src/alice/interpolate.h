...
public:
	...
	Interpolator()
	{
		origin = zVector(0,0,0);
		numPtsPerSegment = nPtsSegment ;
	}

	Interpolator( int _n , zVector _origin = zVector(0,0,0), int _w = 550 , int _h = 150 , float *y =  NULL )
	{
		anchors.resize(_n);
	...
	void performselection(int x, int y, bool &HUDSelecton)
	{
		...
		for (int i = 0; i < anchors.size(); i++)
		{
			float d = anchors[i].distanceTo(zVector(x, y, 0) );

			if (d < nearestD && d < 25 ) id = i;
		}

		if (id != -1 )anchors[id] = zVector(x, y, 0);
	...
	void draw()
	{
		...
		glColor3f(0.75,0.75,0.75);
			...
			for( int i = 0 ; i < anchors.size() ; i++ )
			{
				utils.drawPoint(anchors[i], zColor(1,0,0,1),5);
				sprintf(s,"%1.2f", ofMap(fabs(anchors[i].y-origin.y),0,h,0,1));
		...
		or.y += h*0.1;
		glColor3f(0.75, 0.75, 0.75);
		utils.drawLine(or,or+zVector(w,0,0));
		...
		//requestedVal
		utils.drawLine( zVector(requestedVal,origin.y-h*0.05,0) , zVector(requestedVal,origin.y-h*0.95,0));
