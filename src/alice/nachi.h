...
public:
	...
	{
		for (int i = 0; i < M.n_v; i++)M.positions[i] = EE * M.positions[i];
...
public:
	...
	pathImporter(string file_EndEffector)
	{
		currentPointId = 0;
		Nachi.addMeshes();
	...
	void readPath(string fileToRead = "data/test.txt", string delimiter = ",", float inc = 0)
	{
	...
	void checkPathForReachability()
	{
		...
		for (int i = 0; i < actualPathLength - 1; i++)
		{
			...
			Nachi.rot[1] = ofClamp(Nachi.rot[1], -65, 150);
			Nachi.rot[2] = ofClamp(Nachi.rot[2], -70, 90);
			Nachi.rot[3] = ofClamp(Nachi.rot[3], -170, 170);
			Nachi.rot[4] = ofClamp(Nachi.rot[4], -109, 109);
			Nachi.rot[5] = ofClamp(Nachi.rot[5], -360, 360);*/
	...
	void exportGCode(string fileToWrite = "data/MZ07-01-A.080")
	{
		...
		for (int i = 0; i < actualPathLength - 1; i++)
		{
			...
			// Important, axis 4 and 6
			if (rotations[i - 1][3] < 0 && rotations[i][3] > 0)
				rotations[i][3] -= 360;
	
			if (rotations[i - 1][3] > 0 && rotations[i][3] < 0)
				rotations[i][3] += 360;
		
			if (rotations[i - 1][5] < 0 && rotations[i][5] > 0)
				rotations[i][5] -= 360;
...
*/
	...
	void transformRobotMeshes()
	{
		...
		for (int i = 0; i < E.M.n_v; i++)E.M.positions[i] = Nachi.Bars_to_world_matrices[5] * E.M.positions[i];
	...
	void invertTransformRobotMeshes()
	{
		...
		tw.invert();
		for (int i = 0; i < E.M.n_v; i++)E.M.positions[i] = tw * E.M.positions[i];
	}
